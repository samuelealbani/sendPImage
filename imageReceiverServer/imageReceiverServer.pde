...
void manage_serverNetwork() {
  ...
  case 1:  // the server is created. We send an image request
    ...
    if ( ( nextClient = theServer.available() ) != null ) {  // if there is some client,
      if (nextClient !=null) {
        String whatClientSaid = nextClient.readStringUntil(';');
        if (whatClientSaid != null) {
          println(nextClient.ip() + "\t" + whatClientSaid);
          if(whatClientSaid.equals("sender;")){
             state = 2;  // we go to recieve the length of the image
          }
        }
      }
